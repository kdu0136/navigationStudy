...
fun BottomNavigationView.setupWithNavController(
    ...
    navGraphIds.forEachIndexed { index, navGraphId ->
        if (navGraphId != null) {
            ...
            val navHostFragment = fragmentManager.obtainNavHostFragment(fragmentTag = fragmentTag, navGraphId = navGraphId, containerId = containerId)
            ...
            if (this.selectedItemId == graphId) {
                ...
                fragmentManager.attachNavHostFragment(navHostFragment = navHostFragment, isPrimaryNavFragment = index == 0)
            } else {
                fragmentManager.detachNavHostFragment(navHostFragment = navHostFragment)
            }
        }
    }
    ...
    setOnNavigationItemSelectedListener { item ->
        ...
        if (fragmentManager.isStateSaved) {
            ...
            if (newlySelectedItemTag != null && selectedItemTag != newlySelectedItemTag) {
                ...
                val newSelectedFragment: NavHostFragment = fragmentManager.findFragmentByTag(newlySelectedItemTag) as NavHostFragment
...
//                            R.anim.nav_default_pop_exit_anim)
    ...
    this.setupItemReselected(graphIdToTagMap = graphIdToTagMap, fragmentManager = fragmentManager)

    // Handle deep link
    this.setupDeepLinks(navGraphIds = navGraphIds, fragmentManager = fragmentManager, containerId = containerId, intent = intent)

    // Finally, ensure that we update our BottomNavigationView when the back stack changes
    fragmentManager.addOnBackStackChangedListener {
        if (!isOnFirstFragment && !fragmentManager.isOnBackStack(backStackName = firstFragmentTag)) {
...
private fun FragmentManager.obtainNavHostFragment(fragmentTag: String, navGraphId: Int, containerId: Int): NavHostFragment {
    // If the Nav Host fragment exists, return it
    val existingFragment = this.findFragmentByTag(fragmentTag) as NavHostFragment?
